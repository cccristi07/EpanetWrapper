...
for classid=1:numClass
    ...
    data_ids = find(sqrt(sum(training_feats(:,col_ids).^2)) > 1e-6);   % ensure no zero data elements are chosen
...
Winit = (Xtemp*Xtemp'+eye(size(Xtemp*Xtemp')))\(Xtemp*H_train');
Winit = Winit';

Tinit = (Xtemp*Xtemp'+eye(size(Xtemp*Xtemp')))\(Xtemp*Q');
Tinit = Tinit';
